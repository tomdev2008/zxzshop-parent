...
//        boolean callback = this instanceof FastLinkedHashMap;
        boolean callback = FastLinkedHashMap.class.isInstance(this);
        // Null special case
        if (key == null) {
            Object oldValue;
            if (keyIndexShift > 0) {
    ...
    final V removeKey(Object key, int index) {
        ...
        if (key == null) {
            if (nullKeyPresent) {
                ...
                size--;
                if (FastLinkedHashMap.class.isInstance(this))
     ...
     */
    @SuppressWarnings("unchecked")
    public FastHashMap<K,V> clone() throws CloneNotSupportedException{
