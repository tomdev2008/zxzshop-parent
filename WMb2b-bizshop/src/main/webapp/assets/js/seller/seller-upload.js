(function ($) {
    ...
    function bindEvent(target, opts) {
        ...
        $(target).find('.glyphicon-picture').each(function () {
            ...
            $(this).mouseenter(function (e) {
                ...
                var imgstr = '<div class="lg-view-img"><img src="' + src + '" width="300"></div>';
                if (src != '') {
                    $('body').append(imgstr);
                    ...
                    $('.lg-view-img').css({ 'top':  e.pageY, 'left': e.pageX})
    ...
    var renderView = {
        render: function (opts, container) {
            ...
            html += '<div class="fl"> <p class="text-muted">' + opts.imageDescript + '</p>\</div></div>';
            $(container).html(html);
        }
    };

    $.fn.hishopUpload.defaults = {
        url: '/attachment/upload.do?t=' + Math.random() + "&filePath=transfer",
        displayImgSrc: '',
        title: '图片：',
