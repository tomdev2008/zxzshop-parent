...
$(function(){
    ...
    //提交发布产品
     $('#btnsubmit').click(function(){
        ...
        //};
        var parm =new Object();
        parm.categoryId= $("#categeryid").val();  //类别id
        ...
        $.ajax({
            url: '/product/insertProduct.do?t='+Math.random(),
            type:'post', //数据发送方式
...
function getDefaultEnterprise(){
    $.ajax({
        type:"GET",
        url:"/userinfo/enterpriseinfo.do?categery=1",
        dataType:"json",
        async:false,
            ...
            if (data!=null && data!=undefined && data.code=='000000') {
                 if(data.obj.status!=2){
                 	$.dialog({
                        ...
                        width: '410px',
                        padding: '50px 20px',
                        content: ['<p class="text-center">您好！通过认证即可发布您的商品！</p>'].join(''),
                        ok:function(){
                            if(data.obj && data.obj.id){
                                location.href = "../../pages/seller/AccountCa.html?id="+data.obj.id+"&nav=Account";
                            }else{
                                location.href = "../../pages/seller/Account.html?nav=Account";
                            }       
                        },
                        okVal:'去认证',
                        cancel:function(){}
...
function querymyBrands() {
    $.ajax({
        ...
        type:'post', //数据发送方式
        dataType:'json', //接受数据格式 (这里有很多,常用的有html,xml,js,json)
        data:{"auditStatus":1,"thirdId":$("#hidCategoryIds").val().split("_")[2]},
        success: function(msg){ //成功
