...
function QueryString(name, parent) {
    /// 获取QueryString

    var AllVars = window.location.search.substring(1);
    if(parent){
        var AllVars = window.parent.location.search.substring(1);
    }
...
$(function(){
    //切换框架中页面时，根据nav参数设计父页面中的菜单状态
	var _nav = QueryString('nav');
	if(_nav){
		_nav += '.html';
		$(parent.document).find(".nav.navbar-left ul li a").each(
            function (index, item) {
                if ($(item).attr("href").toLowerCase() == _nav.toLowerCase()) {
                    $(item).parent().click();
                }
            }
        );
    ...
    $(document).on('click', 'a.parent-url', function(){
        var _href = $(this).attr('href');
        if(_href){
            var _index = _href.indexOf('.html');
            var _to = _href.substring(0, _index);
            if(_to){parent.location = "index.html?to="+_to}
        }
