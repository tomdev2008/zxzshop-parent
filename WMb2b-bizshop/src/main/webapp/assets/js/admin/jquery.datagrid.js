...
(function ($) {
    ...
    //内部方法
    //请求Ajax方法
    function request(target, param, jsonp) {
        ...
        var objParams = {};
        objParams[opts.pageNumberName] = opts.pageNumber;
        objParams[opts.pageSizeName] = opts.pageSize;
        var queryObj = $.extend({}, opts.queryParams, objParams);
        ...
        function ajaxRequest(datype) {
            $.ajax({
                type: opts.method,
                url: opts.url,
                data: queryObj,
                dataType: datype ? datype : 'json',
                success: function (data) {
                    ...
                    console.log('ajaxRequest success: ', data);
                },
                error: function (data) {
                    console.log('ajaxRequest error: ', data);
                    setTimeout(function () {
                        $(target).hiMallDatagrid("loaded");
                    }, 0);
    ...
    function getSelectRows(target) {
        ...
        if (opts.idField) {
            ...
            $("tr.hiMallDatagrid-row-selected", tBody).each(function () {
                ...
                rowAry.push(data[opts.backDataName][row_index]);
            });
            return rowAry;
        }
    };
    ...
    function selectAll(target) {
        ...
        var tBody = $(target).find("tbody");
        var rows = data[opts.backDataName];

        $("tr", tBody).addClass("hiMallDatagrid-row-selected");
        $("div.hiMallDatagrid-cell-check input[type=checkbox]", tBody).attr("checked", true);
    ...
    function clearSelectRows(target) {
        ...
        $("div.hiMallDatagrid-cell-check input[type=checkbox]", tBody).attr("checked", false);
        if (opts.idField && data[opts.backDataName] != null) {
            for (var i = 0; i < data[opts.backDataName].length; i++) {
                selectRows.removeById(opts.idField, data[opts.backDataName][i][opts.idField]);
            }
        }
    };
    ...
    function getRowByIndex(target, index) {
    	var opts = $.data(target, "hiMallDatagrid").options;
        var data = $.data(target, "hiMallDatagrid").data;
        if (index < 0 || index >= data[opts.backDataName].length || isNaN(index)) return;
        return data[opts.backDataName][index];
    }
    ...
    function selectRow(target, index) {
        ...
        var tBody = $(target).find("tbody");
        var selectRows = $.data(target, "hiMallDatagrid").selectedRows;
        if (index < 0 || index >= data[opts.backDataName].length) {
            return;
        }
        ...
        if (!tr.hasClass("hiMallDatagrid-row-selected")) {
            ...
            if (opts.idField) {
                var row = data[opts.backDataName][index];
                (function () {
    ...
    function bindCellsEvents(target) {
        var opts = $.data(target, "hiMallDatagrid").options;
        var total = $.data(target, "hiMallDatagrid").data[opts.totalName];
    ...
    function initialPager(target) {
        ...
        if (opts.pagination) {
            ...
            htmlArray.push("<span class=\"active-page\">" + currentPage.toString() + "/" + pageCount.toString() + " 页</span>");
            //htmlArray.push("<span class=\"active-page\">" + currentPage.toString() + "/" + pageCount.toString() + " 页</span>");
    ...
    //插件构造函数
    $.fn.hiMallDatagrid = function (options, param, jsonp) {
        ...
        return this.each(function () {
            ...
            if (state) {
                ...
                var _objs = {
	             ...
	             _objs.data[opts.backDataName] = []; 
	             _objs.data[opts.totalName] = 0; 
                $.data(this, "hiMallDatagrid", _objs);
            }
                ...
                if (opts.loadMsg) {
                    ...
                    */
                    ...
                    if ($("#loadAjax").length === 0) {
                        html.push("<tr id='loadAjax'><td style='height:100px;text-align:center;' colspan='" + opts.columns[0].length.toString() + "'><img src='../../assets/img/dg_loading.gif'></td></tr>");
                    }
    ...
    var renderView = {
        render: function (target, container) {
            var opts = $.data(target, "hiMallDatagrid").options;
            var rows = $.data(target, "hiMallDatagrid").data[opts.backDataName];//$.data(target, "hiMallDatagrid").data.rows;
        ...
        renderHead: function (target, container) {
            ...
            htmlAry.push("<tr id='loadAjax'><th style='height:100px;text-align:center;' colspan='" + closFileds.length.toString() + "'><img src='../../assets/img/dg_loading.gif'></th></tr>");
    ...
    $.fn.hiMallDatagrid.defaults = {
        ...
        pageNumber: 1,
        pageSize: 10,
        pageNumberName:"page", //当前页字段名
        pageSizeName: "rows",  //每页数据条数字段名
        backDataName:"rows",   //后端返回数据的数组字段名，默认为data.rows
        totalName:"total",     //后端返回数据总条数字段名
        queryParams: {},
        sortOrder: "asc",
        view: renderView,
