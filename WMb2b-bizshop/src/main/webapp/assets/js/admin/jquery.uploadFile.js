...
(function ($) {
    function uploadfile(target, opts) {
		...
		if (imgupfile.val() == "") {
		    ...
		    if (!checkImgType(imgupfile.val(), opts.fileType)) {
		        var err = "仅允许上传如下格式文件：";
		        if (opts.fileType.toLowerCase() == "image")
		            err += "gif、jpeg、jpg、png、bmp";
		        else if (opts.fileType.toLowerCase() == "word")
		            err += "doc、docx";
		        else if (opts.fileType.toLowerCase() == "excel")
		            err += "xls、xlsx";
		        else if (opts.fileType.toLowerCase() == "pdf" || opts.fileType.toLowerCase() == "dwg")
		            err += opts.fileType.toLowerCase();
		        else if (opts.fileType.toLowerCase() == "rar")
		            err += "rar、zip";
                else
		            err += "gif、jpeg、jpg、png、bmp、doc、docx、xls、xlsx、pdf、dwg、rar、zip";
			    $.dialog.errorTips(err, '', 3);
				return false;
			}
		}
	...
	//检查上传的图片格式
    function checkImgType(filename, fileType) {
		...
		if (fileType.toLowerCase() == "image")
		    re = new RegExp(/\.(gif|jpg|jpeg|png|bmp)$/);
        else if (fileType.toLowerCase() == "word")
            re = new RegExp(/\.(doc|docx)$/);
		else if(fileType.toLowerCase() == "excel")
		    re = new RegExp(/\.(xls|xlsx)$/);
		else if(fileType.toLowerCase() == "pdf")
		    re = new RegExp(/\.(pdf)$/);
		else if(fileType.toLowerCase() == "dwg")
		    re = new RegExp(/\.(dwg)$/);
		else if(fileType.toLowerCase() == "rar")
		    re = new RegExp(/\.(rar|zip)$/);
	...
	$.fn.himallUpload.defaults = {
        ...
        displayImgSrc:'',
		title: '文件：',
		imageDescript: '请上传正确格式的文件',
		imgFieldName: 'File',
		view: renderView,
		headerWidth: 1,
		dataWidth: 1,
		fileType: 'all' //默认为All，可选类型：Image,Word,Excel,Pdf,Dwg,Rar,All
	};
})(jQuery);