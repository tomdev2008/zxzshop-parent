...
function checkValid() {
    ...
    var t = $("#sendMobileCode").attr("codetype");
    if (t == "sms") {
        return  checkfullnameIsValid()  &  checkMobileIsValid() & checkCheckCodeIsValid() & checkAgreementIsValid();
    }
    else {
    }
}
...
function checkAgreementIsValid() {
    var agreementError = $("#agreement_error");
...
function sendmCode() {
    ...
    jQuery.ajax({
        type: "Post",
        url: _url,
        data: {cellPhone: $("#cellPhone").val(), module: "用户注册",code:$("#checkCode0").val()},
