...
public class StringUtil extends BaseStringUtil {
	 ...
	 */
	private static final String MEDIA_SERVER_URL_DEFAULT = "http://res.eastcailiao.com";
	...
	public static String getUrlFullDefault(String url, boolean sep, String sepOp) {
		if (url!=null && sep && isNotEmpty(sepOp)) {
			if (url.indexOf("|")>0) {
				StringBuilder sBuilder = new StringBuilder();
				String[] urls = url.split("\\|");
				for (int i=0, size=urls.length; i<size; i++) {
